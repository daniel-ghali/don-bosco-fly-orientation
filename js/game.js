...
var SoundManager = (function () {
  var sounds = {};
  var flyAudio = new Audio('sounds/fly-sound.mp3');
  flyAudio.loop = true;
  flyAudio.volume = 0;
  ...
  loadSound('crash', 'sounds/crash-sound.mp3');
  loadSound('rockHit', 'sounds/crash-plane.mp3');
  ...
  return {
    ...
    startEngine: function () {
      flyAudio.play().catch(e => console.log("Audio blocked by browser policy until interaction."));
      flyAudio.volume = 0.15;
    },
    updateEngine: function (speed) {
      if (flyAudio) {

        var referenceSpeed = 0.00038;
        var ratio = speed / referenceSpeed;


        flyAudio.playbackRate = Math.max(0.7, Math.min(1.5, ratio));
        flyAudio.volume = Math.max(0.1, Math.min(0.3, 0.15 * ratio));
      }
    },
    stopEngine: function () {
      if (flyAudio) {
        ...
        var fadeOut = setInterval(() => {
          if (flyAudio.volume > 0.05) {
            flyAudio.volume -= 0.05;
          } else {
            flyAudio.pause();
...
function loop() {
  ...
  airplane.propeller.rotation.x += 0.05 + game.planeSpeed * deltaTime * .001;
...
function updatePlane() {
  ...
  airplane.mesh.position.y += (targetY - airplane.mesh.position.y) * 0.15;
  airplane.mesh.position.x += (targetX - airplane.mesh.position.x) * 0.15;
